...
class Rectangle(Polygon):
    ...
    def __init__(self, center=np.zeros(2), width=100, height=20):
        ...
        v1 = center + np.array([-width/2, height/2])        #upper left
        v2 = center + np.array([width/2, height/2])         #upper right
        v3 = center + np.array([width/2, -height/2])        #lower right
        v4 = center + np.array([-width/2, -height/2])       #lower left
